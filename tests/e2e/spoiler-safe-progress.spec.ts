...
test.describe("Spoiler-safe filtering by reading progress", () => {
  ...
  test("dashboard Recent Discussions hides spoiler threads above viewer progress", async ({ page }) => {
    // Bob is at chapter 6 — the Ch.10 thread "Paul's prescience" must NOT appear
    // in the recent feed even though it's one of the three most-recent threads.
    await loginAs(page, "bob@example.com");
    const club = await getClubByCode("WEDREADS");
    ...
    await expect(page.getByText("Paul's prescience")).toHaveCount(0);

    // Other threads at or below ch.6, plus the untagged thread, can appear.
    // (We assert the exact recent list is spoiler-free rather than a precise
    // ordering, since the recent slice prefers newest first.)
    await expect(page.getByText("Favorite quotes so far")).toBeVisible();
  });
  ...
  test("dashboard Recent Discussions shows higher-chapter threads to readers past that chapter", async ({ page }) => {
    ...
    await expect(page.getByText("Paul's prescience")).toBeVisible();
  });
});
