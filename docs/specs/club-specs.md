...
- `[x]` **CLUB-API-004**: `clubs.join` SHALL require an authenticated caller. In auth v2 identity is always established up front (email OTP → session in entry Step 1), so `clubs.join` is a `protectedProcedure` taking only `{ code }`; an unauthenticated call throws `UNAUTHORIZED`. This removes the prior combined unauthenticated branch (which upserted a user behind the shared passcode) and with it the unverified-user-creation path. (`src/server/routers/clubs.ts`)
...
- `[x]` **CLUB-BE-006**: Owner-only `clubs.archive` sets `status="archived"`; the existing `clubs.update` already rejects writes when `status === "archived"` (the `unarchive` mutation is the explicit re-activation path that flips status back to `"active"`). Both mutations are gated to owner role and emit `FORBIDDEN` for non-owners. Admin-Settings UI surface for archive/unarchive deferred to a follow-up. (`src/server/routers/clubs.ts` `archive`, `unarchive`)
...
- `[x]` **CLUB-UI-SETTINGS-THEME-001**: The admin Settings page SHALL render a "Theme color" section with five curated swatches (Forest Teal — the default, stored as `null`; Library Burgundy; Indigo Manuscript; Slate & Persimmon; Plum Velvet) and a "Custom" tile opening the native `<input type="color">` picker. Picking the Forest Teal swatch SHALL persist `themeColor = null` (inherit global). The currently-saved swatch SHALL be visually selected; an unmatched hex SHALL light the Custom tile. (`src/app/clubs/[clubId]/settings/settings-form.tsx`)
