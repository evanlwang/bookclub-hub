...
- `[x]` **COMP-PROGRESS-BAR-A11Y-001**: The track element SHALL set `role="progressbar"`, `aria-valuenow={Math.min(100, Math.max(0, percentage))}` (clamped to `[0, 100]` for screen-reader correctness only — the visual fill width remains unclamped per `COMP-PROGRESS-BAR-006`), `aria-valuemin="0"`, and `aria-valuemax="100"`.
