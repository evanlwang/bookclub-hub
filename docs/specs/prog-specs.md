...
- `[!]` **PROG-UI-002**: **Replaced.** The page slider and the in-modal `<ProgressBar>` preview were both removed — they were two horizontal bars stacked in a small modal and read as duplicate visuals. The page input and percentage input together cover the same job as plain numeric fields. See `PROG-UI-MODAL-PCT-EDIT-001`.
  - `[!]` **PROG-UI-MODAL-SLIDER-001**: **Removed.** The range slider (`data-testid="page-slider"`) is gone. Page can still be set numerically, and the percentage input drives the page bidirectionally.
...
- `[x]` **PROG-UI-MODAL-PCT-001**: Implemented via PROG-UI-MODAL-PCT-EDIT-001 below.
  - `[x]` **PROG-UI-MODAL-PCT-EDIT-001**: The modal SHALL render an editable percentage number input (`<input type="number">`, `data-testid="percentage-input"`, min 0, max 100) bidirectionally synced with the page input. Changing the percentage SHALL update the page input (`page = round(pct/100 * totalPages)`); changing the page input SHALL update the percentage. Disabled when status="finished" (locked at 100). Editing the percentage from 0 → positive while status="not_started" SHALL auto-bump status to "reading". Page and Progress render as a 2-column grid of plain numeric inputs — no slider, no preview bar.
