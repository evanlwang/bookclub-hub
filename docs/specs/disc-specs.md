...
- `[ ]` **DISC-API-001**: When a member POSTs a new thread with title, body, and optional chapter_tag, the system SHALL create the thread linked to the specified book and club.
- `[ ]` **DISC-API-002**: When a member GETs threads with `max_chapter=N`, the system SHALL return only threads where chapter_number is null OR chapter_number <= N.
- `[ ]` **DISC-API-003**: When a thread author or admin PATCHes a thread, the system SHALL update the specified fields (title, body, chapter_tag, is_pinned).
- `[ ]` **DISC-API-004**: When a thread author or admin DELETEs a thread, the system SHALL delete the thread and all its comments.
...
- `[ ]` **DISC-API-005**: When a member POSTs a comment with optional parent_comment_id, the system SHALL create the comment nested under the parent (or at the top level if no parent).
...
- `[ ]` **DISC-API-006**: When a comment author PATCHes their comment, the system SHALL update the body.
- `[ ]` **DISC-API-007**: When a comment author or admin DELETEs a comment, the system SHALL remove it. Child comments (if any replies exist) shall remain visible with an "[deleted]" placeholder for the parent.
