...
- `[ ]` **AUTH-API-001**: When a user submits email and display_name via `/auth/enter`, the system SHALL create a new user if the email does not exist, or return the existing user if it does.
- `[ ]` **AUTH-API-002**: When an existing user submits via `/auth/enter` with a different display_name, the system SHALL update the display_name to the new value.
...
- `[ ]` **AUTH-API-004**: When a request includes a valid session cookie, the system SHALL return the user's data via `/auth/me` without requiring re-authentication.
- `[ ]` **AUTH-API-005**: When a request includes an invalid or expired session cookie, the system SHALL return 401 from `/auth/me`.
