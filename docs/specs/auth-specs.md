...
- `[ ]` **AUTH-BE-001**: Cookie SHALL be HttpOnly, Secure, SameSite=Lax. **Verified gap (2026-05-07):** the session cookie is set client-side via `document.cookie` (`src/app/join/page.tsx:228`, `src/app/login/page.tsx:66`) — `HttpOnly` cannot be set from JS at all (browsers ignore it), and `Secure` + `SameSite=Lax` are also missing. Fix: emit `Set-Cookie` server-side from `auth.signIn` / `auth.enter` / `clubs.join` with all three attributes; remove the client-side `document.cookie` writes.
- `[ ]` **AUTH-BE-002**: Sliding expiration refreshed on each authenticated request. **Verified gap (2026-05-07):** the refresh logic exists at `src/server/context.ts:27-31` (`prisma.session.update({ expiresAt: computeNewExpiry() })`) but `src/server/context.ts` is **dead code** — zero importers in `src/` or `tests/`. Production routes do their own session lookup without refreshing `expiresAt` (`src/app/api/trpc/[trpc]/route.ts:11-22`, `src/trpc/server.ts:10-21`). Fix: import & use `createContext` from the route handler, or inline the sliding refresh into both lookup paths.
