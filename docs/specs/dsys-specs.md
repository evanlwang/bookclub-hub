...
- `[x]` **DSYS-MOTION-MODAL-001**: Modal surfaces (e.g. `ClubSwitcherModal`) SHALL mount and unmount via `AnimatePresence`. The panel uses the `modalEnter` preset (scale + translate + fade with `slow` duration); the backdrop uses `fadeIn`. Dismissal SHALL play the paired exit before the React tree is removed, so closing animates instead of snap-cutting.
- `[x]` **DSYS-MOTION-TOAST-001**: Toast surfaces (e.g. `SavedToast` in the progress-update flow) SHALL mount and unmount via `AnimatePresence` using the `slideUp` preset, so dismissal animates rather than snap-cuts.
- `[x]` **DSYS-MOTION-CARD-001**: The shared `Card` component SHALL accept an opt-in `animate?: boolean` prop. When `true`, the card SHALL wrap its DOM root in `<motion.div>` and play the `cardEnter` preset via `useMotionPreset`. Default is `false` so existing cards retain SSR-static behavior.
- `[x]` **DSYS-MOTION-BAR-001**: When `animate={true}`, the shared `ProgressBar` SHALL render its fill via `<motion.div>` driven by `scaleX` (transform, GPU-accelerated) rather than animating `width`. The transition SHALL use `tokens.duration.slow` and `tokens.easing.outQuint`, honoring `prefers-reduced-motion` via `useMotionPreset` / `useReducedMotion`. The `delay` prop SHALL continue to offset the start of the fill.
