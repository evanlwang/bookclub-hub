...
- `[x]` **DSYS-FONT-001**: The root layout SHALL load the brand fonts via `next/font/google` — Nunito (display/UI: weights 600,700,800,900), Newsreader (prose: weights 400–700 with italic, `opsz` axis), and Courier Prime (mono: weights 400,700) — each with `display: "swap"`, exposing the CSS variables `--font-nunito`, `--font-newsreader`, `--font-courier-prime` on the document root so the `--font-{display,ui,serif,mono}` stacks in `globals.css` resolve. (`src/app/layout.tsx`)
- `[x]` **DSYS-FONT-002**: The document SHALL NOT load fonts via external `<link>` stylesheets (no Google Fonts `<link>`; Geist removed). `next/font` self-hosts and inlines the font CSS. (`src/app/layout.tsx`)
