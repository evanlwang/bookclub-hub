...
- `[x]` **CAT-BE-EDITIONS-001**: When `getWorkDetail` resolves a work record that passes the popularity gate, the system SHALL fetch the first page of `/works/{key}/editions.json?limit=50` and populate `CatalogBookDetail.isbns` with the deduped union of `isbn_13` and `isbn_10` across editions, capped at the first 10 (most recently indexed). When the work fails the gate, `isbns` SHALL be `[]` and no editions request SHALL be made. (`open-library.ts:241-291`)
- `[x]` **CAT-BE-EDITIONS-002**: The popularity gate SHALL pass when the work record has at least one cover **and** (a description **or** ≥3 subjects). This catches curated works of any era (classics + modern bestsellers) while skipping the bare-skeleton records that make up the long tail of Open Library. Reasoning: such records typically have only one edition and produce no useful ISBN list, so the extra request is wasted bandwidth. (`open-library.ts:271-280`)
- `[x]` **CAT-BE-EDITIONS-003**: Editions responses SHALL be served from a separate in-memory LRU keyed by `editions:{workKey}` with the same TTL/capacity as the search cache. Failures (5xx, timeout, 404) SHALL return `[]` to the caller — a missing editions response is not a fatal error for `getWorkDetail`; the detail view still renders without ISBNs. (`open-library.ts:84` cache instance; `open-library.ts:282-310` non-fatal failure path)
- `[x]` **CAT-UI-DETAIL-ISBN-001**: When `CatalogBookDetail.isbns` is non-empty, the detail panel SHALL render the first up-to-5 ISBNs as a small monospace list under the subjects, labeled "ISBNs". When empty, no list and no label SHALL render. (`catalog-detail-panel.tsx:178-191`)
