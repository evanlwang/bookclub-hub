...
- `[x]` **COMP-ICONS-LOGO-002**: The `LogoIcon` SHALL render the Dogear brand mark — a rounded square (`viewBox="0 0 64 64"`, rx ~15) filled `var(--color-primary)`, with a dog-eared top-right corner: a paper triangle (`var(--color-bg)`, rounded to follow the card corner) over a crease triangle (`var(--color-primary-hover)`) plus a crease stroke. Drawn as plain SVG paths (no `clipPath` element, so multiple instances on a page carry no duplicate `id`). Keeps the `size` prop (default 22) and `aria-hidden="true"`. Mirrors `design_handoff_dogear/landing/assets/dogear-mark.svg`. (`src/components/ui/icons.tsx`)
