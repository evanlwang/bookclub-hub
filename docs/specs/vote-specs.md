...
- `[ ]` **VOTE-API-001**: When an admin POSTs to create a voting round, the system SHALL create it in "nominating" status.
- `[ ]` **VOTE-API-002**: When an admin advances a round from "nominating", the system SHALL transition it to "voting" status.
- `[ ]` **VOTE-API-003**: When an admin advances a round from "voting", the system SHALL transition it to "decided" status, determine the winner, and set the winning book as the club's current book.
...
- `[ ]` **VOTE-API-004**: When an admin cancels a round, the system SHALL set status to "cancelled" and delete all associated nominations and votes.
- `[ ]` **VOTE-BE-002**: The system shall allow only one active round (status "nominating" or "voting") per club at a time.

## Nominations

- `[ ]` **VOTE-API-005**: When a member POSTs a nomination during "nominating" phase, the system SHALL add the book to the round.
- `[ ]` **VOTE-API-006**: When a member attempts to nominate a book already nominated in the same round, the system SHALL return a 409 conflict error.
- `[ ]` **VOTE-API-007**: When a member attempts to nominate during "voting" or "decided" phase, the system SHALL return a 400 error.
- `[ ]` **VOTE-DATA-001**: Each nomination shall have a unique (round_id, book_id) pair.

## Voting

- `[ ]` **VOTE-API-008**: When a member POSTs votes, the system SHALL accept a list of nomination_ids and replace all previous votes for that user in that round.
...
- `[ ]` **VOTE-API-009**: The `/api/books/search` endpoint SHALL query an external book API (Open Library or Google Books) and return results with title, author, cover URL, and page count.
...
- `[ ]` **VOTE-API-010**: The `/api/clubs/:clubId/selections` endpoint SHALL return all books the club has read, ordered by selection date (most recent first).
