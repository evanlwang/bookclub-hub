...
- `[ ]` **VOTE-DATA-VOTE-PERSIST-001**: User votes SHALL persist in the `Vote` table across sessions. On resubmit, the system SHALL atomically replace the user's previous votes for that round: the delete of prior votes and the insert of the new selection SHALL execute in a single database transaction, so a failed resubmit leaves the prior selection intact.
