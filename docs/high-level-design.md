...
Visual judgment — whether a page *feels* right, whether the type scale is coherent, whether the cover gradients harmonize — is not encoded in specs. It lives in the dogear redesign handoff packages (`design_handoff_dogear/app-redesign/`, `design_handoff_dogear/landing/` — interactive prototypes) and any future Figma artifacts, and is referenced from the design LLDs but never round-tripped through EARS.
