...
Visual judgment (does it *feel* right) is not encoded here. The interactive prototypes in the dogear redesign handoff packages (`design_handoff_dogear/app-redesign/`, `design_handoff_dogear/landing/`) are the source of visual truth; LLDs point at them but do not try to reconstruct them.
...
- `design_handoff_dogear/app-redesign/dogear-components.jsx` — visual reference for all primitives.
